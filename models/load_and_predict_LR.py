...
def calculate_lr_predictions(data_path):
    """使用逻辑回归公式计算预测结果，使用原始特征值（不标准化）
    
    Risk score = -1.137 + (0.036 × Feature2) + (0.380 × Feature5) + (0.195 × Feature48) 
    ...
    Probability (P) = e^(Risk score) / (1 + e^(Risk score))
    """
    print("\n1. 加载原始数据...")
    # 加载数据（不进行标准化）
    df = pd.read_excel(data_path) if data_path.endswith('.xlsx') else pd.read_csv(data_path)
    ...
    if 'Label' not in df.columns:
        raise ValueError("数据中没有找到Label列")
    ...
    # 初始化存储所有fold的预测结果
    all_fold_preds = np.zeros(len(df))
    fold_weights = np.zeros(len(df))
    fold_metrics = []
    ...
    print("\n2. 开始交叉验证预测...")
    for fold_idx, (train_idx, val_idx) in enumerate(kfold.split(df, y), 1):
        print(f"\n处理 Fold {fold_idx}/10...")
        ...
        # 计算Risk Score
        risk_score = (-1.137 + 
                     0.036 * val_data['Feature2'].values +
                     0.380 * val_data['Feature5'].values +
                     0.195 * val_data['Feature48'].values +
                     0.016 * val_data['Feature49'].values -
                     0.290 * val_data['Feature50'].values +
                     0.026 * val_data['Feature52'].values -
                     0.168 * val_data['Feature56'].values -
                     0.236 * val_data['Feature57'].values +
                     0.052 * val_data['Feature61'].values +
                     0.018 * val_data['Feature42'].values +
                     0.004 * val_data['Feature43'].values)
        
        # 计算预测概率
        probabilities = np.exp(risk_score) / (1 + np.exp(risk_score))
        
        # 存储当前fold的预测结果
        all_fold_preds[val_idx] = probabilities
        fold_weights[val_idx] = 1
        
        # 计算当前fold的性能
        y_true = val_data['Label'].values
        y_true_tensor = torch.tensor(y_true, dtype=torch.int)
        y_pred_tensor = torch.tensor(probabilities, dtype=torch.float32)
        
        auc_score = auroc(y_pred_tensor, y_true_tensor).item()
        acc_score = accuracy(y_pred_tensor, y_true_tensor).item()
        ...
        print(f"Fold {fold_idx} - AUC: {auc_score:.4f}, ACC: {acc_score:.4f}")
    ...
    # 添加fold信息
    result_df['fold'] = 0
    for fold_idx, (_, val_idx) in enumerate(kfold.split(df, y), 1):
        result_df.loc[val_idx, 'fold'] = fold_idx
    
    result_df.to_csv("results/LR_predictions.csv", index=False)
    print("\n✅ 保存预测结果到 results/LR_predictions.csv")
    
    # 绘制预测分布
    plot_predictions(result_df, result_df['fold'], save_path="results/LR_prediction_distribution.png")
...
if __name__ == "__main__":
    ...
    print("\n开始逻辑回归预测...")
    try:
        lr_results = calculate_lr_predictions("data/AI4healthcare.xlsx")
        if isinstance(lr_results, tuple):
            _, fold_df = lr_results
            print(f"\n逻辑回归最终结果:")
            print(f"Mean AUC: {fold_df['auc'].mean():.4f} ± {fold_df['auc'].std():.4f}")
            print(f"Mean ACC: {fold_df['acc'].mean():.4f} ± {fold_df['acc'].std():.4f}")
    except Exception as e:
